...
void testOrderExecution() {
    ...
    // Assign initial ownership and armies
    tA->setOwner(p1); p1->addTerritory(tA); tA->setArmies(5);
    tB->setOwner(p2); p2->addTerritory(tB); tB->setArmies(0); 
    tC->setOwner(p2); p2->addTerritory(tC); tC->setArmies(3);

    // Display initial game state
    cout << "Territories and owners:\n";
    ...
    // Setup deck with predefined cards
    Deck deck;
    ...
    // 3) ONE CARD MAX per turn 
    cout << "\n3) Card award test (max 1 card per turn)\n";
    if (p1->hasConqueredThisTurn()) {
...
if (tC->getOwner() == GameEngine::getNeutralPlayer()) {
    ...
    // 6) All orders issued and executed
    cout << "\n6) Issue and execute all order types\n";
    OrdersList allOrders;
    allOrders.addOrder(Deploy(p1, tA, 2).clone());
    ...
    // Cleanup (Not deleting map to avoid territory double-free)
    delete p1;
    delete p2;
...
/**
 * @brief Main function to run Order execution tests.
 */
int main() {
    testOrderExecution();
    return 0;
